...
import Effect.Promise.Console as Promise
import Effect.Ref as Ref
import Effect.Timer (setTimeout)
import Foreign.Object (Object)
import Foreign.Object as FO
import Data.Number (atan2, cos, sin)
import Web.Event.Event (EventType(EventType))
import Web.Event.EventTarget (eventListener, addEventListener)
import Web.PointerEvents.PointerEvent as PointerEvent
import Web.DOM.Element (setAttribute, toEventTarget) as Element
...
import Peer as Peer
import PendingPeer (PendingPeer(PendingPeer), createPendingPeer)
import PendingPeer as PendingPeer
import Signaling (PeerType(Operator))
...
main :: Effect Unit
main = do
  peer <- createPendingPeer Operator
  ...
  initActuatorControl "primary" "KeyA" "KeyD" Gamepad.Left Gamepad.Right setSecondary
  pure unit

sendMessage :: PendingPeer -> Object Number -> Effect Unit
sendMessage pendingPeer message =
  PendingPeer.sendMessage pendingPeer (fromObject (fromNumber <$> message))
...
sendRequest :: Deferred => PendingPeer -> Object Number -> Promise Unit
sendRequest (PendingPeer pendingPeer) request = do
  peer <- liftEffect (Ref.read pendingPeer)
  sendRequest' peer
  where
    sendRequest' :: Deferred => _ -> Promise Unit
...
initJoystick getControl' = do
  ...
  joystickElement <- unsafePartial <<< (\x -> fromJust x) <$> querySelector (QuerySelector "#joystick-position") (HTMLDocument.toParentNode document)
...
initActuator id getControl' = do
  document <- window >>= document
  up <- unsafePartial <<< (\x -> fromJust x) <$> querySelector (QuerySelector ("#" <> id <> "-up")) (HTMLDocument.toParentNode document)
  down <- unsafePartial <<< (\x -> fromJust x) <$> querySelector (QuerySelector ("#" <> id <> "-down")) (HTMLDocument.toParentNode document)
  zeroTimeoutRef <- Ref.new Nothing
...
initJoystickControl up down left right axisH axisV set = do
  document <- window >>= document
  joystick <- (unsafePartial <<< \x -> fromJust x) <$> querySelector (QuerySelector "#joystick") (HTMLDocument.toParentNode document)
...
initActuatorControl id up down gamepadUp gamepadDown set = do
  document <- window >>= document
  actuatorUp <- (unsafePartial <<< \x -> fromJust x) <$> querySelector (QuerySelector ("#" <> id <> "-up")) (HTMLDocument.toParentNode document)
  actuatorDown <- (unsafePartial <<< \x -> fromJust x) <$> querySelector (QuerySelector ("#" <> id <> "-down")) (HTMLDocument.toParentNode document)
  setByMouseRef <- Ref.new false
  keyboardStateRef <- Ref.new Set.empty
  gamepadDeferredRef <- Ref.new false
  gamepadStateRef <- Ref.new (Tuple 0.0 0.0)
